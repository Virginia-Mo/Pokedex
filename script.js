...
getInputValue() {
    window.inputNumber = document.getElementById("searchBar_input").value.toLowerCase();
    if (inputNumber) {
...
async getEvolutions() {
    app.getImgInfos()
    try {
        let response = await fetch(`https://pokeapi.co/api/v2/pokemon-species/${pokemon.id}`);
